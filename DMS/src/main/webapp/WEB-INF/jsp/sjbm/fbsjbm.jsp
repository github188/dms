...
<div class="top">发布数据编码</div>
<div class="main">
当前数据库位置：<input type="text" id="tableName" value="${tableName }" readonly="readonly">
<p align="center">  <button class="zjsjbm">数据编码发布</button> <input id="submitValue"type="text" value="请输入关键词" onFocus="if(value==defaultValue){value='';this.style.color='#000'}" onBlur="if(!value){value=defaultValue;this.style.color='#999'}" style="color:#999999" > <button class="query">查询</button></p>
    	...
    	<c:if test="${not empty sjbmList.list}">
						...
						<c:forEach items="${sjbmList.list}" var="sjbm" varStatus="vs">
							...
							<tr class="gradeX odd">
								...
								<td class=" "><c:choose><c:when test="${sjbm.f_CM_SJBM_fbzt==0}"><a href="SjbmAction/fbsjbm.do?id=${sjbm.f_CM_SJBM_id}">未发布</a></c:when><c:otherwise>已发布</c:otherwise></c:choose>
								</td>
							</tr>
						</c:forEach>    
					</c:if>
...
<p align="center">
			...
			<c:if test="${sjbmList.hasPreviousPage}">
                        <td><a href="${pageContext.request.contextPath}/SjbmAction/fbzt.do?pageNum=${sjbmList.prePage}">前一页</a></td>
                    </c:if>
                    <c:forEach items="${sjbmList.navigatepageNums}" var="nav">
                        ...
                        <c:if test="${nav != sjbmList.pageNum}">
                            <td><a href="${pageContext.request.contextPath}/SjbmAction/fbzt.do?pageNum=${nav}">${nav}</a></td>
                        </c:if>
                    </c:forEach>
                    <c:if test="${sjbmList.hasNextPage}">
                        <td><a href="${pageContext.request.contextPath}/SjbmAction/fbzt.do?pageNum=${sjbmList.nextPage}">下一页</a></td>
                    </c:if>
</p>
</div>
<div class="footer">
